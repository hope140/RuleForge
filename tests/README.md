...
- 同一 selector 命中不同策略时按 `security > explicit override > specificity > category semantics > same-category source preference > fallback` 裁决；Blackmatrix 只可在同一 category 内作为来源 tie-breaker，语义覆盖关系保留双方并记录顺序约束。
...
- GitHub Copilot、Grok、Bing/Copilot、gvt1、Netflix/Apple 共享基础设施、Prime Video 和 Trae/MarsCode 的业务边界应通过生成后的路由样例复核。
- 共享基础设施根域审计应记录 category、rule、risk、reason 和 source_id，并允许明确的服务 hostname 继续参与正常审计。
