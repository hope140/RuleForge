...
模板默认只监听本机，`secret` 为空，TUN 与 IPv6 关闭，DNS 使用 Fake-IP。模板同时启用 GeoSite 中国域名数据库，避免 Fake-IP 流量只能依赖 `GEOIP,CN`。业务策略组排在地区测速组之前，图标来自 Orz-3 `mini/Color`。完整使用说明见 [Mihomo 模板说明](profiles/mihomo/README.md)。
...
Mihomo 模板在明确的业务和代理规则之后使用 `GEOSITE,cn,DIRECT` 作为中国域名兜底，再使用 `GEOIP,CN,DIRECT` 处理已经拿到真实目标 IP 的连接。
